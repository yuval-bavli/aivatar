...
public static class BakeEyebrows4
{
    [MenuItem("Aivatar/Bake Eyebrows 4")]
    public static string Run()
    {
        var log = new System.Text.StringBuilder();

        // Load ORIGINAL face texture (not baked)
        ...
        Color browColor = new Color(0.20f, 0.14f, 0.10f);

        // Right eyebrow (viewer's left) - thicker, stronger, multiple strokes
        for (int pass = 0; pass < 5; pass++)
        {
            float yOff = pass * 4f - 8f; // Spread vertically for thickness
            float str = 0.7f - pass * 0.08f;
            PaintBrowArc(tex,
        ...
        // Left eyebrow (viewer's right) - mirror
        for (int pass = 0; pass < 5; pass++)
        {
            float yOff = pass * 4f - 8f;
            float str = 0.7f - pass * 0.08f;
            PaintBrowArc(tex,
        ...
        // Add individual hair-like strokes for natural look
        var rng = new System.Random(42);
        Color hairColor = new Color(0.18f, 0.12f, 0.08f);
        for (int i = 0; i < 40; i++)
        {
            ...
            ry = ry + (peak - ry) * 0.3f + (float)(rng.NextDouble() - 0.5) * 16;
            PaintStroke(tex, (int)rx, (int)ry, 4 + rng.Next(6), (float)(rng.NextDouble() * 0.3 + 0.1), hairColor, 0.5f);

            // Left brow (mirror)
            float lx = Mathf.Lerp(1362, 1402, t) + (float)(rng.NextDouble() - 0.5) * 10;
            float ly = ry;
            PaintStroke(tex, (int)lx, (int)ly, 4 + rng.Next(6), (float)(rng.NextDouble() * 0.3 + 0.1), hairColor, 0.5f);
        }

        log.AppendLine("Painted thick eyebrow arcs with hair strokes");
        ...
        for (int i = 0; i <= steps; i++)
        {
            ...
            for (int dy = -ir; dy <= ir; dy++)
            {
                for (int dx = -ir; dx <= ir; dx++)
                {
                    ...
                    tex.SetPixel(px, py, Color.Lerp(existing, color, alpha));
                }
            }
        }
    }
