...
/// BakeEyebrows9 — near-invisible feather per qwen3-vl:8b audit:
///   - colour:    (0.60, 0.46, 0.32)  was (0.50, 0.38, 0.26)
///   - strength:  0.05                 was 0.09
///   - thickness: 2px                  was 3px
///   - passes:    1
public static class BakeEyebrows9
{
    [MenuItem("Aivatar/Bake Eyebrows 9 (near-invisible)")]
    public static string Run()
    {
        ...
        Color browColor = new Color(0.60f, 0.46f, 0.32f);

        PaintBrowArc(tex, 698, 1455, 658, 1310, 1510, 2f, browColor, 0.05f);
        PaintBrowArc(tex, 1362, 1455, 1402, 1310, 1510, 2f, browColor, 0.05f);

        // Very minimal hair strokes
        var rng = new System.Random(42);
        Color hairColor = new Color(0.55f, 0.42f, 0.30f);
        for (int i = 0; i < 5; i++)
        {
            ...
            PaintStroke(tex, (int)rx, (int)ry, 1 + rng.Next(2), (float)(rng.NextDouble() * 0.3 + 0.1), hairColor, 0.04f);
            float lx = Mathf.Lerp(1362, 1402, t) + (float)(rng.NextDouble() - 0.5) * 6;
            PaintStroke(tex, (int)lx, (int)ry, 1 + rng.Next(2), (float)(rng.NextDouble() * 0.3 + 0.1), hairColor, 0.04f);
        }
        ...
        return "BakeEyebrows9: near-invisible arcs painted";
    }
