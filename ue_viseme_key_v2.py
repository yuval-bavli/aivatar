...
v2: NO key_all_zero — only key the controls each viseme actually uses.
The control rig rest pose handles defaults.

Each viseme gets keys ONLY for the controls it uses.
At frame 0 (sil) we explicitly key all used controls to 0 to establish a baseline.
"""
import unreal
...
# First clear ALL existing keys from the section channels
face_section = None
...
def sf(name, frame, val):
    try:
        lib.set_local_control_rig_float(level_seq, face_rig, name,
            unreal.FrameNumber(frame), float(val), time_unit=DT, set_key=True)
...
def sv2(name, frame, x, y):
    try:
        lib.set_local_control_rig_vector2d(level_seq, face_rig, name,
            unreal.FrameNumber(frame), unreal.Vector2D(float(x), float(y)),
            time_unit=DT, set_key=True)
    except Exception as e:
        errors.append(f"{name}@{frame}: {e}")

# Define each viseme as a dict of {control: value}
# Vector2D controls use tuples (x, y)
viseme_poses = {
    0: {  # sil - rest
        "CTRL_C_jaw": (0, 0),
    },
...
out.append("\nKeying visemes...")
for frame, controls in sorted(viseme_poses.items()):
...
if face_section:
    ...
    for ch in channels:
        if 'jaw.Y' in ch.get_name():
            keys = list(ch.get_keys())
            out.append(f"\nJaw.Y channel ({ch.get_name()}): {len(keys)} keys")
            for k in keys:
