"""Take close-up screenshots of mouth area for frames 0, 30, 100."""
...
# Focus on lower face / mouth area
# Viewport region: roughly center of the viewport, lower half
vx1 = x + int(w * 0.25)
vy1 = y + int(h * 0.25)
vx2 = x + int(w * 0.45)
vy2 = y + int(h * 0.42)

frames = [(0, "sil (rest)"), (30, "TH (teeth+tongue)"), (100, "aa (open jaw)")]
cell_w, cell_h = 500, 450
...
for i, (frame, name) in enumerate(frames):
    ...
    draw.text((10, 5), f"F{frame}: {name}", fill=(255, 255, 100))
    cell.paste(img, (0, 28))
    grid.paste(cell, (i * cell_w, 0))

grid.save("c:/Users/yuval/src/aivatar/ue_mouth_compare.png")
